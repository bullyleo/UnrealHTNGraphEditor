...
class SToolTip;
class SVerticalBox;
class UHTNGraphEditorGraphNode;

//--------------------------------------------------------------
// グラフエディタに表示されるノードのレイアウトとそれに関連する動作を定義する
//--------------------------------------------------------------
class SGraphNode_HTNGraph : public SGraphNodeAI
{
public:
	SLATE_BEGIN_ARGS(SGraphNode_HTNGraph) {}
	SLATE_END_ARGS()

	void Construct(const FArguments& InArgs, UHTNGraphEditorGraphNode* InNode);
